...
# ── Image Processing (PIL only --- no cv2!) ──────────────────────
def load_image_pil(uploaded_file):
    """
    Load image using PIL only.
    Handles 8-bit and 16-bit PNG from clinical X-ray scanners.
    No cv2 required!
    """
    try:
        ...
        # Handle 16-bit PNG from medical scanners
...
def detect_and_draw(model, uploaded_file, conf):
    """
    Run YOLOv8 detection and draw boxes using PIL.
    Saves temp file for ultralytics, draws with PIL.
    No cv2 required!
    """
    # Save to temp file for ultralytics
    tmp = tempfile.NamedTemporaryFile(
        suffix='.png', delete=False
    )

    # Load and save as PNG using PIL
    img_pil = load_image_pil(uploaded_file)
    if img_pil is None:
        return None, None, []
    ...
    img_draw = img_draw.resize((640, 640))
    ...
    if res.boxes is not None:
        for box in res.boxes:
            ...
            # Get coordinates scaled to display size
            ...
            color = CLASS_COLORS_RGB.get(nm, (150,150,150))
            lw = 4 if nm == 'fracture' else 2
            ...
            try:
                ...
                lw_px, lh_px = len(label) * 7, 12

            label_y = max(y1 - lh_px - 6, 0)
            draw.rectangle(
                [x1, label_y, x1 + lw_px + 8, label_y + lh_px + 4],
                fill=color
            )
            draw.text(
                (x1 + 4, label_y + 2),
...
st.markdown(
    ...
    "(JPG, JPEG, PNG including 16-bit clinical PNG). "
    "KAN-YOLOv8 detects all **9 pathological classes** "
...
if uploaded_files:
    ...
    if st.button(
        ...
        for idx, uf in enumerate(uploaded_files):
            ...
            img_result, elapsed, dets = detect_and_draw(
                model, uf, conf
            )
        ...
        if tot_frac > 0:
            ...
            ⚠️ {tot_finds} finding(s) — no fractures. Clinical review recommended.
            </span>
            </div>
            """, unsafe_allow_html=True)
        else:
            ...
            border-radius:12px;padding:14px 20px;margin:12px 0;">
            <span style="color:#4ADE80;font-weight:700;">
            ✅ No findings at {conf:.0%} confidence threshold.
            </span>
            </div>
            """, unsafe_allow_html=True)
        ...
        for i, r in enumerate(results):
            ...
            if has_frac:
                badge = f"🚨 {len(r['fractures'])} FRACTURE(S)"
            elif has_find:
            ...
            with st.expander(
                ...
                if has_frac:
                    ...
                    <span style="color:#EF4444;font-weight:700;">
                    🚨 {len(r['fractures'])} fracture(s) detected!
                ...
                if r['dets']:
                    ...
                    for j, det in enumerate(r['dets']):
                        ...
                        with cols[j % 3]:
                            ...
                            font-size:13px;">{em} {cls.upper()}</div>
                            <div style="color:{col};font-size:20px;
                            font-weight:800;margin:4px 0;">{cf:.0%}</div>
                ...
                ⚡ KAN-YOLOv8: <b style="color:#94A3B8;">
                {r['elapsed']:.3f}s</b> &nbsp;|&nbsp;
                Manual: <b style="color:#94A3B8;">10–15 min</b>
                &nbsp;|&nbsp;
        ...
        spd_t = int(600/avg_t) if avg_t > 0 else 15000
        s3.metric("Speed Improvement",  f"{spd_t:,}×")
...
# ── Welcome Screen ───────────────────────────────────────────────
    ...
    with ca:
        ...
        Recall: 0.635<br>Speed: 0.04s</div></div>
        """, unsafe_allow_html=True)
    with cb:
        st.markdown("""
        <div style="background:#1E293B;border:1px solid #334155;
        ...
        Recall: 0.605<br>Speed: 0.04s</div></div>
        """, unsafe_allow_html=True)
    with cc:
        st.markdown("""
        <div style="background:#1E293B;border:1px solid #334155;
        ...
        Recall: 0.653<br>Speed: 0.04s</div></div>
        """, unsafe_allow_html=True)
    with cd:
        st.markdown("""
        ...
        Recall: 0.618<br>Speed: 0.04s</div></div>
        """, unsafe_allow_html=True)
    ...
    with h1:
        ...
        Upload 1–10 wrist X-ray images
        (JPG, PNG, 16-bit clinical PNG)</div></div>
        """, unsafe_allow_html=True)
    with h2:
        st.markdown("""
        <div style="background:#1E293B;border-radius:12px;
        ...
        Step 2 — Detect</div>
        <div style="color:#94A3B8;font-size:12px;">
        Click Detect — KAN-YOLOv8 analyzes
        all images in 0.04s each</div></div>
        """, unsafe_allow_html=True)
    with h3:
        st.markdown("""
        <div style="background:#1E293B;border-radius:12px;
        ...
        confidence scores and alerts</div></div>
        """, unsafe_allow_html=True)
    ...
    Comparative study of YOLOv8, YOLOv12, and RT-DETR on
    GRAZPEDWRI-DX (20,327 pediatric wrist radiographs, 9 classes).
    External validation on de-identified radiographs from
    <b style="color:#60A5FA;">Driscoll Children's Hospital</b>,
    Corpus Christi, Texas.
    Dept. of Mathematics & Statistics, TAMUCC.
    </div></div>
    """, unsafe_allow_html=True)
